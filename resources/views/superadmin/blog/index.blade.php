    ...
    <div class="container mt-4">
        <div class="d-flex justify-content-between align-items-center mb-3">
            ...
            <a href="{{ route('blog-admin-create') }}" class="btn btn-custom-2 btn-sm">
                <i class="fas fa-plus"></i> {{ __('messages-superadmin.sidebar.blog.add_article') }}
            </a>
        </div>
        <table class="table table-bordered">
            <thead class="field-title">
                <tr>
                    <th>No</th>
                    <th>{{ __('messages-superadmin.sidebar.blog.title') }}</th>
                    <th>{{ __('messages-superadmin.sidebar.blog.author') }}</th>
            ...
            <tbody>
                @foreach ($blogs as $data)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('blog-admin-edit', $data->id) }}" class="btn btn-custom-3 my-2" title="Edit"
                                data-bs-toggle="tooltip">
                                <i class="fas fa-edit text-primary"></i>
                            </a>

                            <form action="{{ route('blog-admin-delete', $data->id) }}" method="POST"
