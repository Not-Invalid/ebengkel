...
@section('content')
    <div class="w-100 shadow bg-white rounded" style="padding: 1rem">
        <h4>Edit {{ __('messages-superadmin.sidebar.info_support_center.info') }}</h4>
        <form class="py-4" action="{{ route('support-center-info-update', $supportInfo->id) }}" method="POST"
                ...
                <div class="col">
                    <div class="did-floating-label-content">
                        <select class="did-floating-input" id="support_category_id" name="support_category_id" required>
                            <option value="" disabled>{{ __('messages-superadmin.sidebar.blog.select_category') }}
                            </option>
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}" @if ($category->id == $supportInfo->support_category_id) selected @endif>
