...
@section('content')
                ...
                @foreach ($cartItems as $item)
                    <div class="cart-item d-flex align-items-center mb-3 p-3"
                        ...
                        <div class="cart-item-image me-3">
                            @if ($item->produk)
                                <img src="{{ optional($item->produk)->foto_produk ? url($item->produk->foto_produk) : asset('assets/images/components/image.png') }}" alt="Product Image" style="width: 100px; height: 100px; object-fit: cover;" />
                            @elseif ($item->sparepart)
                                <img src="{{ optional($item->sparepart)->foto_spare_part ? url($item->sparepart->foto_spare_part) : asset('assets/images/components/image.png') }}" alt="Spare Part Image" style="width: 100px; height: 100px; object-fit: cover;" />
                            @endif
                        </div>
                        <div class="cart-item-details flex-grow-1">
                            <h6>{{ optional($item->produk)->nama_produk ?? optional($item->sparepart)->nama_spare_part ?? 'Item Tidak Ditemukan' }}</h6>
                            <p class="text-muted mb-1">{{ optional($item->produk)->bengkel->nama_bengkel ?? 'Nama Bengkel Tidak Tersedia' }}</p>
                        ...
                        <div class="cart-item-quantity d-flex align-items-center me-3">
                            ...
                            <input type="text" value="{{ $item->quantity }}" class="form-control form-control-sm text-center mx-1 quantity-input" style="width: 50px;">
                            <button class="btn btn-outline-dark btn-sm increment-quantity">+</button>
                        </div>
                        <button class="btn btn-sm btn-outline-danger remove-item p-1" data-id="{{ $item->id }}"><i class='bx bx-trash'></i></button>
                    </div>
    ...
    document.querySelector('.fw-semibold').innerText = `${selectedItemsCount} Selected Item${selectedItemsCount > 1 ? 's' : ''}`;
}
