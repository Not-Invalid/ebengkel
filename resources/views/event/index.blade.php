    ...
    <div class="container">
      <div class="row">
        <div class="col-md-12 text-center">
          <h4 class="title-header">{{ __('messages.event.see_our') }}</h4>
        </div>
      </div>
    </div>
  </section>
            ...
            <form method="GET" action="{{ route('event.show') }}" style="width: 60%;">
              <div class="input-group">
                <input type="text" name="search" value="{{ request('search') }}"
                  placeholder="{{ __('messages.event.search') }}..." class="form-control"
              ...
              <div class="d-flex justify-content-center pb-5">
                <div class="text-center">
                  <img src="{{ asset('assets/images/components/empty.png') }}" height="200" width="200"
                    alt="Empty">
                  @if (request('search'))
                    <p>{{ __('messages.event.not_found') }} "{{ request('search') }}".</p>
                  @else
                    <p>{{ __('messages.event.no_data') }}.</p>
                  @endif
                </div>
              </div>
            @endif
              ...
              <ul class="pagination">
                ...
                @if ($events->onFirstPage())
                  <li class="page-item disabled">
                    <a class="page-link">{{ __('messages.event.previous') }}</a>
                  </li>
                @else
                  <li class="page-item">
                    <a href="{{ $events->previousPageUrl() }}" class="page-link">{{ __('messages.event.previous') }}</a>
                  </li>
                @endif
                ...
                @if ($events->hasMorePages())
                  <li class="page-item">
                    <a href="{{ $events->nextPageUrl() }}" class="page-link">{{ __('messages.event.next') }}</a>
                  </li>
                @else
                  <li class="page-item disabled">
                    <a class="page-link">{{ __('messages.event.next') }}</a>
                  </li>
                @endif
              </ul>
            </nav>
