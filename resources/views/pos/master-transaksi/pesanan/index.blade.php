        ...
        <div class="card-body">
          <div class="d-flex justify-between">
            ...
            <div class="d-flex justify-end mb-4">
              <form action="{{ route('pos.transaksi_pesanan.create', $bengkel->id_bengkel) }}">
                @csrf
                ...
                @else
                  @foreach ($orders as $index => $order)
                    <tr>
                      ...
                      <td>{{ $order->pegawai ? $order->pegawai->nama_pegawai : 'Online Order' }}</td>
                      <td>{{ $order->nama_pemesan }}</td>
                      <td>{{ $order->telp_pelanggan }}</td>
                      <td>{{ $order->service ? $order->service->nama_services : 'No Service' }}</td>
                      <td>{{ ucfirst($order->status) }}</td>
                      <td>Rp {{ number_format($order->total_pesanan, 2, ',', '.') }}</td>
                      <td class="d-flex justify-content-center align-items-center gap-4">
                        <a href="{{ route('pos.transaksi_pesanan.edit', $bengkel->id_bengkel) }}"
                          class="btn btn-sm btn-primary">
                          <i class="fas fa-edit"></i> Edit
                        </a>
                        <form action="{{ route('pos.transaksi_pesanan.delete', $bengkel->id_bengkel) }}" method="POST"
                          class="delete-form">
                          @csrf
                          @method('DELETE')
