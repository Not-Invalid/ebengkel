...
@section('content')
        ...
        <div class="card-body">
          <div class="d-flex justify-between">
            ...
            <div class="d-flex justify-end mb-4">
              <a href="{{ route('pos.management-stock.opname.create', $bengkel->id_bengkel) }}"
          ...
          <div class="table-responsive bg-white rounded shadow">
            <table class="table table-bordered table-striped text-center">
              ...
              <tbody id="stock-opname-table-body">
                @forelse ($combined as $stock)
                  <tr>
                    <td>{{ $combined->firstItem() + $loop->iteration - 1 }}</td>
                    <td>{{ $stock->product_brand }}</td>
                    <td>{{ $stock->product_name }}</td>
                    <td>{{ $stock->type === 'product' ? 'Product' : 'Spare Part' }}</td>
                    <td>{{ $stock->recorded_quantity }}</td>
                    <td>{{ $stock->actual_quantity }}</td> <!-- Actual Quantity -->
                    <td>
                      {{-- Difference calculation --}}
                      <span class="text-danger">
                        {{ $stock->actual_quantity - $stock->recorded_quantity }}
                      </span>
                    </td>
                    ...
                    <td class="d-flex justify-content-center align-items-center">
                      <form
                        action="{{ route('pos.management-stock.opname.delete', ['id_bengkel' => $bengkel->id_bengkel, 'id_opname' => $stock->id_opname]) }}"
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      filterTable('opname-table-body', [1, 2]);
    });
  </script>
