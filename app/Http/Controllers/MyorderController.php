...
class MyorderController extends Controller
{
    public function index(Request $request)
    {
        ...
        $ordersQuery = OrderOnline::with(['orderItems.produk.fotoProduk', 'orderItems.sparepart.fotoSparepart', 'invoice', 'bengkel', ])->where('id_pelanggan', auth('pelanggan')
            ->id())
            ->where('is_delete', 'N')
        ...
        $orders = $orders->map(function ($order)
        {
                ...
                ->orderItems->map(function ($orderItem)
            {
                ...
                if ($orderItem->produk && $orderItem
                    ->produk
                    ->fotoProduk)
                {
                    $imagePath = $orderItem
                        ->produk
                        ->fotoProduk->file_foto_produk_1;
                }

                elseif ($orderItem->sparepart && $orderItem
                    ->sparepart
                    ->fotoSparepart)
                {
                    $imagePath = $orderItem
                        ->sparepart
                        ->fotoSparepart->file_foto_spare_part_1;
                }
    ...
    public function detailOrder($orderId)
    {
        ...
        $order = OrderOnline::with(['orderItems.produk.fotoProduk', 'orderItems.sparepart.fotoSparepart', 'invoice', 'bengkel', ])->where('order_id', $orderId)->where('is_delete', 'N')
            ->firstOrFail();
